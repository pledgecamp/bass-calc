...
pub fn bass_fn_point(data: &BassFnData, w: f64) -> f64 {
    let num = data.num.iter().enumerate().fold(0, |s, (i, v)| s + poly_calc(v, w, i as f64));
    let den = data.den.iter().enumerate().fold(0, |s, (i, v)| s + poly_calc(v, w, i as f64));
    let n = num / den;
    n.norm_sqr().sqrt()
}
...
pub fn Radiator(params: &Parameters) -> BassFnData {
    ...
    let α = Parameters.α.v();
    let δ = Parameters.δ.v();
    let psi = α + P.δ.v() + 1.0;
    let y = Parameters.y.v();
    let y2 = y.sqrt();
    let Qmp = Parameters.Qmp.v();
    let Qs = Parameters.Qs.v();

    let T0 = Parameters.Ts.v() / (y2 * g25); // 8a
    let a1 = (y2 / g25) * 
